{
  ...
  collectMimeAssociations = let
    ...
    defaultHandlerEntries = lib.flatten (lib.mapAttrsToList (appKey: appCfg: let
      desktopCfg = appCfg.desktopFile or {};
    ...
    defaultApplications = lib.mapAttrs (_mime: entries:
      if entries != []
      then (lib.last entries).desktop
      else null)
    groupedByMime;
    addedAssociations = let
      allHandlers = lib.flatten (lib.mapAttrsToList (appKey: appCfg: let
        desktopCfg = appCfg.desktopFile or {};
        mimeTypes = desktopCfg.defaultAssociations or [];
        appInfo = appCfg.appInfo or {};
        displayName = desktopCfg.displayName or appInfo.name or appKey;
    ...
    in
      lib.mapAttrs (_mime: entries: lib.unique (map (e: e.desktop) entries)) grouped;
  in {inherit defaultApplications addedAssociations;};
