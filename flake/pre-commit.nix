            ...
            entry = let
              validateScript = pkgs.writeShellScript "validate-flatpaks" ''
                ...
                while IFS= read -r file; do
                  ...
                  # Check if this file explicitly defines a flatpak type
                  has_flatpak_type=$(${pkgs.ripgrep}/bin/rg 'type\s*=\s*"flatpak"' "$file" 2>/dev/null || true)

                  # If it explicitly defines a different type (like pwa), skip it
                  has_other_type=$(${pkgs.ripgrep}/bin/rg 'type\s*=\s*"(pwa|web-page|nix|blank|externally-managed|custom)"' "$file" 2>/dev/null || true)

                  # Extract all IDs from the file
                  ids=$(${pkgs.ripgrep}/bin/rg -o 'id\s*=\s*"([^"]+)"' -r '$1' "$file" 2>/dev/null || true)
