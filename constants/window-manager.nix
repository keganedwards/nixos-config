{
  ...
  windowManager = username: rec {
    ...
    setKeybindings = bindings: {
      home-manager.users.${username}.programs.niri.settings.binds = lib.mapAttrs (key: cmd: {
        action.spawn = 
    ...
    withConfig = attrs: {
      ...
      attrs
      (if keybindings != {} then setKeybindings keybindings else {})
      (if startup != [] then setStartup startup else {})
      (if windowRules != [] then setWindowRules windowRules else {})
      (if packages != [] then { home-manager.users.${username}.home.packages = packages; } else {})
      (if settings != {} then setSettings settings else {})
    ];
  };
in windowManager
